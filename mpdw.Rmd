...
library(rio)
data1 <- import("https://raw.githubusercontent.com/hafizdharka/MPDWT/main/Covid%2019%20Confirmed%20Cases-Kerala.csv")
data1
```
...
```{r}
ts.plot(data1.ts, xlab="Time Period ", ylab="Kasus Terkonfirmasi", 
...
training_ma <- data1[1:672,]
testing_ma <- data1[673:841,]
train_ma.ts <- ts(training_ma$Confirmed)
test_ma.ts <- ts(testing_ma$Confirmed)
```
...
library(ggplot2)
ggplot() + 
  geom_line(data = training_ma, aes(x = Date, y = Confirmed, col = "Data Latih")) +
  geom_line(data = testing_ma, aes(x = Date, y = Confirmed, col = "Data Uji")) +
  labs(x = "Periode Waktu", y = "Kasus Terkonfirmasi", color = "Legend") +
...
```{r}
ts.plot(data1.ts, xlab="Time Period ", ylab="Kasus Terkonfirmasi", main= "SMA N=4 Kasus Terkonfirmasi")
points(data1.ts)
...
```{r}
ts.plot(data1.ts, xlab="Time Period ", ylab="Kasus Terkonfirmasi", main= "DMA N=4 Data Kasus Terkonfirmasi")
points(data1.ts)
...
training<-data1[1:672,]
testing<-data1[673:841,]
train.ts <- ts(training$Confirmed)
test.ts <- ts(testing$Confirmed)
```
...
library(ggplot2)
ggplot() + 
  geom_line(data = training, aes(x = Periode, y = Confirmed, col = "Data Latih")) +
  geom_line(data = testing, aes(x = Periode, y = Confirmed, col = "Data Uji")) +
  labs(x = "Periode Waktu", y = "Kasus Terkonfirmasi", color = "Legend") +
  scale_colour_manual(name="Keterangan:", breaks = c("Data Latih", "Data Uji"),
                      values = c("blue", "red")) + 
  theme_bw() + theme(legend.position = "bottom",
...
```{r}
#Cara 1 (fungsi ses)
ses.1 <- ses(train.ts, h = 169, alpha = 0.2)
plot(ses.1)
ses.1

ses.2<- ses(train.ts, h = 169, alpha = 0.7)
plot(ses.2)
ses.2
```
...
autoplot(ses.1) +
  ...
  ylab("Kasus Terkonfirmasi") + xlab("Periode")
```
...
```{r}
#SES
ses.opt <- ses(train.ts, h = 169, alpha = NULL)
plot(ses.opt)
ses.opt
...
#ramalan
ramalanopt<- forecast(sesopt, h=169)
ramalanopt
```
...
resid1<-training$Confirmed-ramalan1$fitted
head(resid1)
```
...
resid2<-training$Confirmed-ramalan2$fitted
head(resid2)
...
```{r}
selisih1<-ramalan1$mean-testing$Confirmed
SSEtesting1<-sum(selisih1^2)
MSEtesting1<-SSEtesting1/length(testing)

selisih2<-ramalan2$mean-testing$Confirmed
SSEtesting2<-sum(selisih2^2)
MSEtesting2<-SSEtesting2/length(testing)

selisihopt<-ramalanopt$mean-testing$Confirmed
...
#ramalan
ramalandesopt<- forecast(des.opt, h=169)
ramalandesopt
```
